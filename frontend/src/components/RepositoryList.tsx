                ...
                {sortedRepos.slice(0, 6).map((repo) => (
                    <div key={repo.name} className="p-6 bg-white rounded-2xl border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] transition-all duration-300 flex flex-col h-full group">
                        ...
                        {repo.recommendations && repo.recommendations.length > 0 && (
                            <div className="mb-6 bg-amber-50/50 border border-amber-100 p-3 rounded-xl">
                                <div className="flex items-center gap-2 mb-2">
                                    <svg className="w-3 h-3 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    <h4 className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">Analysis</h4>
                                </div>
